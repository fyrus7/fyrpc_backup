...
function apiUrl() {
  return String(GAS_API || "").replace(/\/+$/, "");
}

function getGasAction(path) {
  ...
  const map = {
    ...
    walkin: "walkin",
    "walkin-categories": "getWalkinCategories",
    getWalkinCategories: "getWalkinCategories",
...
async function gasRequest(path, options = {}) {
  const action = getGasAction(path);
  const queryData = parseQuery(path);
  const bodyData = parseBody(options);
  ...
  */
  if (action === "search") {
    if (payload.ic && !payload.term) payload.term = payload.ic;
    if (payload.mode && !payload.option) payload.option = payload.mode;
  }
...
async function apiJson(path, options = {}) {
  const raw = await gasRequest(path, options);
  ...
  try {
    ...
    if (err.message && raw.includes(err.message)) {
      throw err;
    }
...
// CARD HAS BEEN MOVE TO INDIVIDUAL JS FOR COMPATIBILITY
...
function loadSummaryCard() {
  apiJson("/summary")
    ...
    .catch(err => {
      console.error(err);
      document.getElementById("result").innerHTML =
        "<span style='color:red;'>Failed to load summary</span>";
    });
}

async function showCategoryBalance() {
  try {
    const data = await apiJson("/category-balance");

    if (!data.success) throw new Error("CATEGORY_BALANCE_FAILED");
